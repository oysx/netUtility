#!/bin/bash
# run on client side(eth0: 10.10.20.1, want to connect to 10.35.53.1):
# $0 172.77.7.2 10.10.10.1 10.35.53.1
# run on server side(eth0: 10.10.10.1, connected to subnet 10.25.53.0/24):
# $0 172.77.7.1 10.10.20.1

if $(( $# < 2 ));then
	echo "Usage: $0 local_tunnel_ip remote_real_ip [dest_net]"
	exit 1
fi
...
tunnel_ip=$1
remote_ip=$2
dest_net=$3	#destination subnet want to access via tunnel
